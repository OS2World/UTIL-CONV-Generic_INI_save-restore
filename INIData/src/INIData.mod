...
IMPLEMENTATION MODULE INIData;
        ...
        (*                                                          *)
        (*    INIData Started:      30 March 2000                   *)
        (*    DLL version started:  7 April 2019                    *)
...
PROCEDURE ChooseDefaultINI (appname: ARRAY OF CHAR;
    ...
    BEGIN
        ...
        app := "$SYS";
        hini := OpenINIFile (Iname);
        foundI :=INIGet (hini, app, "UseTNI", useTNI0);
        CloseINIFile (hini);

        hini := OpenINIFile (Tname);
...
PROCEDURE OpenINIFile (VAR (*IN*) filename: ARRAY OF CHAR): HINI;
    ...
    VAR result: HINI;  length: CARDINAL;  UseTextFile, failure: BOOLEAN;
    ...
    BEGIN
        ...
        IF length < 3 THEN
            UseTextFile := FALSE;
        ELSE
            UseTextFile := CAP(filename[length-3]) = 'T';
        END (*IF*);
...
PROCEDURE SetInitialWindowPosition (hwnd: OS2.HWND;
    ...
    BEGIN
        ...
        IF hini = NIL THEN
            hini := CreateINIFile(INIFileName);
        END (*IF*);
        app := "WindowPos";
